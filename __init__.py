...
class BuildGraph(object):
    ...
    def getFinalVertices(self):
        ...
        for v in self.vertices:
            ...
            if len(froms) is 0 and 0 < len(tos):
                finals.append(v)
        return finals
    ...
    def traverse(self, v, level=0):
        ...
        oi = [e["operation"] for e in ei]
        assert len(set(oi)) is 1, "Vertex %s must be generated by identical edge operations" % v
    ...
    def buildVertex(self, v, ei):
        ...
        if operation == "compile":
            ...
            assert len(vi) is 1, "Compile operations are 1-to-1 transformations ('%s')" % v
...
def init(root):
    """Creates a new build_graph.csv table in the given folder. (An error is
       ...
       Also creates a default epic_package.json file.
    """
    ...
    with open(bgPath, 'w') as f:
        ...
        dw.writeheader()
        dw.writerows(edges)
    # we also write out a epic_package.json with default "UNI", "semver", and "dependency" properties
    epPath = os.path.abspath(root) + "/epic_package.json"
    with open(epPath, 'w') as f:
        json.dump({
            "uni": "com.github.%s.%s" % (name, name),
            "semver": "0.0.1",
            "dependencies": []
        }, f, indent=4)
